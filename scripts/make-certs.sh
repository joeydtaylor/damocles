...
# using ../etc/keys/tls/san.cnf as the source SAN config.
# Usage: ./scripts/make-certs.sh
#
# Idempotency:
#   - If ../etc/keys/tls/ca.crt and ca.key exist, CA is reused.
...
echo "     To force a new CA (rotates trust), run: FORCE_CA=1 ./scripts/make-certs.sh"
